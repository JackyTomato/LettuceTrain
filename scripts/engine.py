...
def train_step(model, dataloader, loss_fn, optimizer, scaler, device):
    ...
    for batch, (data, labels) in enumerate(loop):
        ...
        pred_class = torch.argmax(torch.softmax(preds, dim=1), dim=1)
        train_acc += (pred_class == labels).sum().item() / len(preds)
...
def test_step(model, dataloader, loss_fn, device):
    ...
    The test step does not apply a sigmoid function, thus a loss function appropiate for logits
    must be used.
    ...
    with torch.inference_mode():
        ...
        for batch, (data, labels) in enumerate(dataloader):
            ...
            # 2. Calculate and accumulate loss (logits haven't been through sigmoid!)
...
def train(
    model, train_dataloader, test_dataloader, optimizer, scaler, loss_fn, epochs, device
):
